...
const cardData = [
  ...
  },
  {
    ...
    description: '6 students, Gold & Silver',
    label: 'Achievement',
    emoji: '🏆',
    images: [
  ...
  },
  {
    ...
    description: 'Bangalore, Feb 2025',
    label: 'Community',
    emoji: '👨‍👩‍👧',
    images: [
  ...
  },
  {
    ...
    description: 'Live class screenshots',
    label: 'Learning',
    emoji: '📚',
    images: [
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="card-responsive grid gap-2">
          {cardData.map((card, index) => {
            const baseClassName = `card flex flex-col justify-between relative aspect-[4/3] min-h-[200px] w-full max-w-full p-0 rounded-[20px] border border-solid font-light overflow-hidden transition-all duration-300 ease-in-out hover:-translate-y-0.5 ${
            ...
            const cardContent = (
              <>
                {/* Drag stack of photos */}
                <div style={{ position: 'absolute', inset: 0, zIndex: 0, overflow: 'hidden' }}>
                  <Stack
                    randomRotation={false}
                    sensitivity={200}
                    sendToBackOnClick={true}
                    cards={card.images.map((src, i) => (
                      <img 
                        key={i} 
                        src={src} 
                        alt={`card-${i + 1}`} 
                ...
                <div style={{ 
                  position: 'absolute', 
                  inset: 0, 
                  background: 'linear-gradient(to top, rgba(18, 15, 23, 0.95) 0%, rgba(18, 15, 23, 0.4) 60%, rgba(18, 15, 23, 0.8) 100%)', 
                  zIndex: 2, 
                  display: 'flex', 
                  ...
                  <div className="card__header flex justify-between gap-3 relative text-white items-center w-full">
                    <span className="card__label text-xs uppercase tracking-wider font-semibold opacity-75">{card.label}</span>
                    {card.emoji && <span style={{ fontSize: 20 }}>{card.emoji}</span>}
                  </div>
                  <div className="card__content flex flex-col relative text-white mt-auto">
                    <h3 className="card__title font-semibold text-[15px] m-0 mb-0.5">
                      {card.title}
                    </h3>
                    <p className="card__description text-[12px] opacity-75 m-0">
                      {card.description}
                    </p>
                  </div>
                </div>
